...
export async function login() {
    try {
        ...
        const brosweResult = await openAuthSessionAsync(
            respone.toString(),
            redirectUri
        );

        if (brosweResult.type !== "success") throw new Error("Failed to login");

        const url = new URL(brosweResult.url);
...
export async function getCurrenUser() {
    try {
        ...
        if (respone.$id) {
            const userAvatar = await avatar.getInitials(respone.name);
